...
static bool arg_headless_cryptsetup = false;
...
static int parse_proc_cmdline_item(const char *key, const char *value, void *data) {
        ...
        } else if (streq(key, "mount.crypt_headless")) {

                r = value ? parse_boolean(value) : 1;
                if (r < 0)
                        log_warning_errno(r, "Failed to parse crypt_headless switch \"%s\", ignoring: %m", value);
                else
                        arg_headless_cryptsetup = r;
